...
- `judgement.ts` — the same pair for the answer cross-check (`verdict` / `confidence` are enums; `readVerdict` maps prose from schema-less rungs onto them). `artifactToText` in `solution.ts` flattens a solution for the judge.

### Client (`src/`)

- `hooks/use-solve.ts` — per-provider state machine (`idle → waiting → streaming → done | error`) over `lib/sse.ts`. Providers run through `runPool(…, SOLVE_CONCURRENCY, …)` — 2, because the answer cross-check runs its two solvers together and nothing runs more (it was 1 on the free plan; see the CPU section of `AGENTS.md`). A stream that closes with no terminal event is retried once after the wave. With a judge set, `start(providers, body, judge)` follows the wave with `/api/judge` and exposes it as `judgeRun`; the solutions go as Solution A/B in picker order.
