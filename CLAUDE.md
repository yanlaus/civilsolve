...
- `providers.ts` — `ProviderKey` / `ChannelKey` registries, defaults (`DEFAULT_PROVIDER`, `DEFAULT_INTERPRETERS`, `DEFAULT_VERIFIER`), `HealthResponse` shape. `PROVIDER_KEYS` is every provider (the reader and judge lists); `SOLVER_KEYS` leaves out `REVIEW_ONLY_PROVIDERS` (Gemini, since 26 September 2026), and only those get solver cards - the worker refuses a solve for the rest.
