...
- **`jobs.ts`** - `TaskJob` runs `runTask` into a hub sink that never rejects, so the run outlives the page; the stream opens with `event: job {id}`; the final event is stored and an alarm deletes it 24 h later. Images are never stored.
