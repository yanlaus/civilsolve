...
- **Keep API keys server-side only.** `POE_API_KEY`, `KIMI_API_KEY`, and `MINIMAX_API_KEY` are the only secrets; they live in `.dev.vars` locally and in Wrangler secrets in production. Never expose them to frontend code.
...
- **Kimi and MiniMax route through `worker/openai-compat.ts`** (OpenAI chat-completions protocol; per-provider endpoint/model/key resolved in `worker/upstream.ts`), the Poe providers through `worker/poe.ts`; all share the SSE shell in `worker/run.ts`. The default Kimi Code endpoint rejects Cloudflare-Workers-origin traffic at its WAF — the documented fallback is switching `KIMI_API_URL` to the Moonshot platform endpoint, not spoofing client headers.
