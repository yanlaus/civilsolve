...
- **`kimi-k2.7-code` misreads diagrams.** On an overhanging beam with a 4 m partial UDL it read the UDL as 6 m and returned reactions of 13.75/36.25 kN instead of 10/30, in 127 s. `kimi-k3` read the same image correctly in 28 s. Do not switch the default back without re-running that fixture.
